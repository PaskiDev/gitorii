        ...
        match &self.command {
            ...
            Commands::Pipeline { action, remote } => {
                ...
                let (platform, owner, repo_name, api_base_url) =
                    crate::pr::detect_platform_full(&repo_path, remote)
                    .ok_or_else(|| anyhow::anyhow!(
                ...
                let client = crate::pipeline::get_pipeline_client_with_base_url(
                    &platform, Some(&api_base_url),
                )?;
                match action {
            ...
            Commands::Platforms { action } => {
                ...
                match action {
                    PlatformsCommands::List => {
                        ...
                        for e in &user {
                            println!("* {:<20} {:<22} {:<26} {}", e.name, e.kind, e.domain, e.api_base_url);
                        }
                        for b in reg::builtins() {
                            if user_names.contains(b.name.as_str()) { continue; }
                            println!("  {:<20} {:<22} {:<26} {}", b.name, b.kind, b.domain, b.api_base_url);
                        }
                        println!();
                        println!("(* = custom · `torii platforms add` to add; `torii platforms remove` to drop)");
                    }
                    PlatformsCommands::Add {
                        name, kind, domain, api_base_url, web_base_url, client_id, local,
                    } => {
                        const KINDS: &[&str] = &[
                            "gitlab", "gitea", "forgejo", "codeberg",
                            "github", "github_enterprise",
                            "bitbucket", "bitbucket_data_center",
                        ];
                        if !KINDS.contains(&kind.as_str()) {
                            anyhow::bail!(
                                "unknown kind `{}`. Known: {}", kind, KINDS.join(", ")
                            );
                        }
                    ...
                    PlatformsCommands::Remove { name, local } => {
                        ...
                        if removed {
                            ...
                            if reg::builtins().iter().any(|b| &b.name == name) {
                                ...
                                println!("(no platform named `{}` in the {} registry)",
                                    name, if *local { "local" } else { "global" });
                            }
                        }
                    }
                    PlatformsCommands::Test { name } => {
                        // Find the entry (custom > builtin).
                        let entry = reg::all(".").into_iter()
                            .find(|e| &e.name == name)
                            .ok_or_else(|| anyhow::anyhow!(
                                "no platform named `{}`. Run `torii platforms list`.", name
                            ))?;
                        ...
                        let token = crate::auth::resolve_token(provider, ".").value
                            .ok_or_else(|| anyhow::anyhow!(
                        ...
                        if (200..300).contains(&status) {
                            println!("✓ {} → {} {}", entry.api_base_url, status, resp.status().canonical_reason().unwrap_or(""));
                        } else {
                            let body = resp.text().unwrap_or_default();
                            anyhow::bail!(
                                "✗ {} → {}: {}",
                                entry.api_base_url, status, body.chars().take(200).collect::<String>()
                            );
                        }
                    }
                }
