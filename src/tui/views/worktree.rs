...
pub fn refresh(app: &mut App) {
    ...
    if let Ok(names) = repo.worktrees() {
        for i in 0..names.len() {
            let name = match names.get(i) { Some(n) => n, None => continue };
            let wt = match repo.find_worktree(name) { Ok(w) => w, Err(_) => continue };
            let path = wt.path().canonicalize().unwrap_or_else(|_| wt.path().to_path_buf());
...
fn describe(path: &std::path::Path) -> (String, String) {
    ...
    let dirty = repo.statuses(Some(&mut so)).ok()
        .map(|ss| ss.iter()
            .filter(|s| !s.status().contains(git2::Status::IGNORED))
            .count())
        .unwrap_or(0);
    let state = if dirty == 0 { "clean".to_string() } else { format!("{} change(s)", dirty) };
    (branch, state)
}
...
pub fn render(f: &mut Frame, app: &App, area: Rect) {
    ...
    match app.worktree_view.focus {
        ...
        WorktreeFocus::ConfirmRemove => render_confirm(f, app, area, "Remove the selected worktree?"),
        WorktreeFocus::ConfirmPrune  => render_confirm(f, app, area, "Prune stale worktree admin dirs?"),
        WorktreeFocus::List          => {}
    }
}
...
fn render_list(f: &mut Frame, app: &App, area: Rect) {
    ...
    let items: Vec<ListItem> = if app.worktree_view.items.is_empty() {
        vec![ListItem::new(Span::styled("  no worktrees", Style::default().fg(C_DIM)))]
    } else {
        app.worktree_view.items.iter().enumerate().map(|(i, w)| {
            ...
            ListItem::new(Line::from(vec![
                ...
                Span::styled(format!("{:<22}", w.name), Style::default().fg(C_WHITE).add_modifier(Modifier::BOLD)),
    ...
    if !app.worktree_view.items.is_empty() { state.select(Some(app.worktree_view.idx)); }
    let title_color = if focused && app.worktree_view.focus == WorktreeFocus::List { C_WHITE } else { bc };
...
fn render_detail(f: &mut Frame, app: &App, area: Rect) {
    ...
    let body: Vec<Line> = if let Some(w) = app.worktree_view.items.get(app.worktree_view.idx) {
        vec![
            ...
            kv("state",  &w.state,  if w.state == "clean" { C_GREEN } else { C_YELLOW }),
            Line::from(""),
...
pub fn ops_for(state: &WorktreeState) -> Vec<(&'static str, &'static str)> {
    let is_main = state.items.get(state.idx).map(|w| w.is_main).unwrap_or(false);
    let locked = state.items.get(state.idx).map(|w| w.state.starts_with("locked")).unwrap_or(false);

    let mut ops: Vec<(&'static str, &'static str)> = Vec::new();
    ops.push(("Add new worktree",     "create + check out a branch in a sibling dir"));
...
fn render_input_overlay(f: &mut Frame, app: &App, area: Rect) {
    ...
    let body = vec![
        Line::from(Span::styled(format!(" {}", app.worktree_view.input_prompt), Style::default().fg(C_WHITE))),
        Line::from(""),
        Line::from(vec![
            Span::raw("  "),
            Span::styled(&app.worktree_view.input_buffer, Style::default().fg(C_WHITE)),
...
fn render_confirm(f: &mut Frame, app: &App, area: Rect, prompt: &str) {
    ...
    f.render_widget(Clear, popup);
    let body = vec![
        Line::from(Span::styled(format!("  {}", prompt), Style::default().fg(C_WHITE))),
        Line::from(""),
