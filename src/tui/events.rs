...
// directly.
fn handle_submodule(key: event::KeyEvent, app: &mut App) -> Option<Action> {
    ...
    match app.submodule_view.focus {
        SubmoduleFocus::List => {
            match (key.modifiers, key.code) {
                (_, KeyCode::Up) | (_, KeyCode::Char('k')) => {
                    if app.submodule_view.idx > 0 { app.submodule_view.idx -= 1; }
                }
                (_, KeyCode::Down) | (_, KeyCode::Char('j')) => {
        ...
        SubmoduleFocus::InputArgs => {
            match (key.modifiers, key.code) {
                (_, KeyCode::Enter) => {
                    ...
                    match op {
                        SubmodulePendingOp::AddUrl => {
                            ...
                            app.submodule_view.input_prompt = "path (relative to repo root)".to_string();
                        ...
                        SubmodulePendingOp::AddPath => {
                            ...
                            let opts = crate::cmd::submodule::AddOpts {
                                branch: None, name: None, recursive: false,
                            };
                            match crate::cmd::submodule::add(&repo_path, &url, &path, &opts) {
                                Ok(_)  => app.set_status(format!("✓ submodule added at `{}`", path.display())),
                ...
                (_, KeyCode::Backspace) => { app.submodule_view.input_buffer.pop(); }
        ...
        SubmoduleFocus::ConfirmRemove => {
            match (key.modifiers, key.code) {
                (_, KeyCode::Char('y')) | (_, KeyCode::Char('Y')) => {
                    let path = app.submodule_view.items.get(app.submodule_view.idx)
...
fn dispatch_submodule_op(app: &mut App) -> Option<Action> {
    use crate::tui::app::{SubmoduleFocus, SubmodulePendingOp};
    use std::path::Path;
    let ops = crate::tui::views::submodule::ops_for(&app.submodule_view);
    let label = ops.get(app.submodule_view.dropdown_idx).map(|o| o.0).unwrap_or("");
    let repo = Path::new(".");
    match label {
        "Add new submodule" => {
        ...
        "Update" => {
            let opts = crate::cmd::submodule::UpdateOpts { init: false, recursive: false };
        ...
        "Update + init" => {
            let opts = crate::cmd::submodule::UpdateOpts { init: true, recursive: false };
            match crate::cmd::submodule::update(repo, &opts) {
        ...
        _ => { app.submodule_view.focus = SubmoduleFocus::List; }
    }
    None
}
...
fn handle_worktree(key: event::KeyEvent, app: &mut App) -> Option<Action> {
    ...
    match app.worktree_view.focus {
        WorktreeFocus::List => {
            match (key.modifiers, key.code) {
                (_, KeyCode::Up) | (_, KeyCode::Char('k')) => {
                    if app.worktree_view.idx > 0 { app.worktree_view.idx -= 1; }
                }
                (_, KeyCode::Down) | (_, KeyCode::Char('j')) => {
        ...
        WorktreeFocus::InputArgs => {
            match (key.modifiers, key.code) {
                    ...
                    match op {
                        WorktreePendingOp::AddBranch => {
                            ...
                            let res = crate::cmd::worktree::add(
                                &repo_path, spec,
                                &crate::cmd::worktree::AddOpts { explicit_path: None },
                            );
                            match res {
                                Ok(_)  => app.set_status(format!("✓ worktree added for `{}`", trimmed)),
                                Err(e) => app.set_status(format!("✗ {}", e)),
                            }
                        }
                        WorktreePendingOp::LockReason => {
                            let target = app.worktree_view.items.get(app.worktree_view.idx)
                        ...
                        WorktreePendingOp::MoveNewPath => {
                            let cur = app.worktree_view.items.get(app.worktree_view.idx)
                                .map(|w| std::path::PathBuf::from(&w.path));
                            if let Some(c) = cur {
                                let new = std::path::PathBuf::from(trimmed);
                                match crate::cmd::worktree::move_wt(&repo_path, &c, &new) {
                                    Ok(_)  => app.set_status(format!("✓ moved to `{}`", new.display())),
                                    Err(e) => app.set_status(format!("✗ {}", e)),
                                }
                            }
                        }
                ...
                (_, KeyCode::Backspace) => { app.worktree_view.input_buffer.pop(); }
                (_, KeyCode::Char(c)) if key.modifiers != KeyModifiers::CONTROL => {
                ...
                (_, KeyCode::Char('y')) | (_, KeyCode::Char('Y')) => {
                    ...
                    if let Some(t) = target {
                        let res = crate::cmd::worktree::remove(
                            Path::new("."), &t,
                            &crate::cmd::worktree::RemoveOpts { force: false, no_snapshot: false },
                        );
                        match res {
...
fn dispatch_worktree_op(app: &mut App) -> Option<Action> {
    use crate::tui::app::{WorktreeFocus, WorktreePendingOp};
    use std::path::Path;
    let ops = crate::tui::views::worktree::ops_for(&app.worktree_view);
    let label = ops.get(app.worktree_view.dropdown_idx).map(|o| o.0).unwrap_or("");
    let repo = Path::new(".");
    match label {
        "Add new worktree" => {
        ...
        "Open in $SHELL" => {
            ...
            // a separate task.
            let path = app.worktree_view.items.get(app.worktree_view.idx)
        ...
        _ => { app.worktree_view.focus = WorktreeFocus::List; }
    }
    None
}
