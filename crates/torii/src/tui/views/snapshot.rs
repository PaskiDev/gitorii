...
pub fn render(f: &mut Frame, app: &App, area: Rect) {
    ...
    let items: Vec<ListItem> = if app.snapshot_view.snapshots.is_empty() {
        ...
        display_indices
            ...
            .map(|&i| {
                ...
                let is_sel = in_list
                    && app.snapshot_view.idx
                        == display_indices
                            .iter()
                            .position(|&x| x == i)
                            .unwrap_or(usize::MAX);
                let name_color = if !app.snapshot_view.search_query.is_empty()
    ...
    if in_list && !display_indices.is_empty() {
    ...
    let title = if app.snapshot_view.search_mode {
        ...
        format!(
            " snapshots — \"{}\"  {} matches ",
            app.snapshot_view.search_query,
            display_indices.len()
        )
    } else {
        format!(" snapshots ({}) ", app.snapshot_view.snapshots.len())
    };
    let title_color = if app.snapshot_view.search_mode {
        C_YELLOW
    } else if in_list {
        C_WHITE
    } else {
    ...
    let list_block = Block::default()
        .title(Span::styled(
            ...
            Style::default().fg(title_color).add_modifier(
                if in_list || app.snapshot_view.search_mode {
                    Modifier::BOLD
                } else {
                    Modifier::empty()
                },
    ...
    let auto_items: Vec<ListItem> = intervals
        .iter()
        .enumerate()
        .map(|(i, interval)| {
    ...
    let auto_block = Block::default()
        .title(Span::styled(
            format!(
                ...
                app.snapshot_view.auto_interval.label()
            ),
            if in_auto {
                Style::default().fg(C_WHITE).add_modifier(Modifier::BOLD)
            } else {
