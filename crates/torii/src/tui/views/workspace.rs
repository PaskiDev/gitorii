...
pub fn render(f: &mut Frame, app: &App, area: Rect) {
    ...
    let ws_items: Vec<ListItem> = app
        ...
        .map(|(i, ws)| {
            ...
            let sync_color = if ahead_total > 0 || behind_total > 0 {
                C_YELLOW
            } else {
                C_GREEN
            };
            ...
            ListItem::new(Line::from(vec![
                ...
                if dirty > 0 {
                    Span::styled(format!("*{}", dirty), Style::default().fg(C_YELLOW))
                } else {
                    Span::raw("")
                },
            ]))
    ...
    f.render_stateful_widget(
        List::new(repo_items).block(repos_block),
        cols[1],
        &mut repo_state,
    );
    ...
    let info_lines: Vec<Line> = if focus_repos {
        if let Some(ws) = app.workspace_view.workspaces.get(app.workspace_view.ws_idx) {
            if let Some(r) = ws.repos.get(app.workspace_view.repo_idx) {
                ...
                let sync_color = if r.ahead > 0 || r.behind > 0 {
                    C_YELLOW
                } else {
                    C_GREEN
                };
        ...
        if let Some(ws) = app.workspace_view.workspaces.get(app.workspace_view.ws_idx) {
            ...
            vec![
                ...
                Line::from(vec![
                    ...
                    Span::styled(
                        format!("{}", ahead),
                        Style::default().fg(if ahead > 0 { C_CYAN } else { C_DIM }),
                    ),
                ]),
                Line::from(vec![
                    ...
                    Span::styled(
                        format!("{}", behind),
                        Style::default().fg(if behind > 0 { C_RED } else { C_DIM }),
                    ),
                ]),
                Line::from(vec![
                    Span::styled("  dirty   ", Style::default().fg(C_SUBTLE)),
                    Span::styled(
                        format!("{}", dirty),
                        Style::default().fg(if dirty > 0 { C_YELLOW } else { C_DIM }),
                    ),
                ]),
            ]
        } else {
