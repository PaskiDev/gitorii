        ...
        terminal.draw(|f| {
            ...
            let tab_line = Line::from(vec![
                ...
                Span::styled(
                    "[1] ",
                    Style::default()
                        .fg(if tab_repos_active {
                            C_WHITE
                        } else {
                            BRAND_COLOR
                        })
                        .add_modifier(if tab_repos_active {
                            Modifier::BOLD
                        } else {
                            Modifier::empty()
                        }),
                ...
                Span::styled(
                    ...
                    Style::default().fg(if tab_repos_active {
                        C_WHITE
                    } else {
                        BRAND_COLOR
                    }),
                ...
                Span::styled(
                    "[2] ",
                    Style::default()
                        .fg(if tab_ws_active { C_WHITE } else { BRAND_COLOR })
                        .add_modifier(if tab_ws_active {
                            Modifier::BOLD
                        } else {
                            Modifier::empty()
                        }),
            ...
            match tab {
                PickerTab::Repos => {
                    ...
                    let items: Vec<ListItem> = repos
                        ...
                        .map(|(i, p)| {
                            ...
                            ListItem::new(Line::from(vec![
                                ...
                                Span::styled(
                                    check,
                                    Style::default().fg(if is_sel { C_GREEN } else { C_DIM }),
                                ),
                                Span::styled(
                                    format!("{:<24}", name),
                                    Style::default().fg(if is_cur { C_WHITE } else { C_SUBTLE }),
                                ),
                    ...
                    let name_content = match mode {
                        PickerMode::NamingWorkspace => {
                            ...
                            let after = if cursor >= ws_name.len() {
                                ""
                            } else {
                            ...
                            Line::from(vec![
                                ...
                                Span::styled(
                                    cur_char.to_string(),
                                    Style::default().bg(BRAND_COLOR).fg(C_WHITE),
                                ),
                    ...
                    f.render_widget(
                        Paragraph::new(name_content).block(
                            Block::default()
                ...
                PickerTab::Workspaces => {
                    if saved_ws.is_empty() {
                        ...
                        let ws_list_items: Vec<ListItem> = saved_ws
                            .iter()
                            .enumerate()
                            .map(|(i, ws)| {
                                let is_cur = i == ws_idx;
                        ...
                        let left_border = if !ws_panel_right {
                            C_WHITE
                        } else {
                            BRAND_COLOR
                        };
                        ...
                        f.render_stateful_widget(
                            List::new(ws_list_items).block(
                                Block::default()
                                    .title(Span::styled(
                                        " workspaces ",
                                        Style::default().fg(left_border),
                                    ))
                                    .borders(Borders::ALL)
                                    .border_type(border_type)
                        ...
                        let repo_items: Vec<ListItem> = saved_ws
                            .get(ws_idx)
                            .map(|ws| {
                                ws.repos
                                    .iter()
            ...
            match tab {
                PickerTab::Repos => match mode {
                    PickerMode::Selecting => match (key.modifiers, key.code) {
                        ...
                        (KeyModifiers::SHIFT, KeyCode::Char('A')) | (_, KeyCode::Char('A')) => {
                            selected.iter_mut().for_each(|s| *s = false);
                        }
                        (_, KeyCode::Enter) => {
                            let sel_repos: Vec<PathBuf> = repos
                                .iter()
                                .enumerate()
                                ...
                                .collect();
                            if sel_repos.len() == 1 {
                                break PickerResult::SingleRepo(
                                    sel_repos.into_iter().next().unwrap(),
                                );
