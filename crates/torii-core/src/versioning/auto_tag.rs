...
use crate::versioning::conventional::{CommitType, ConventionalCommit};
...
impl AutoTagger {
    ...
    pub fn calculate_next_version(&self, commit_msg: &str) -> Result<Option<Version>> {
        ...
        let current_version = self.get_latest_version()?.unwrap_or_else(Version::initial);
    ...
    pub fn calculate_next_version_from_log(&self) -> Result<Option<Version>> {
        ...
        for oid in revwalk.filter_map(|r| r.ok()) {
            ...
            if let Ok(commit) = self.repo.repo.find_commit(oid) {
                ...
                match bump {
                    VersionBump::Major => {
                        highest = VersionBump::Major;
                        break;
                    }
                    VersionBump::Minor if highest != VersionBump::Major => {
                        highest = VersionBump::Minor;
                    }
                    VersionBump::Patch if highest == VersionBump::None => {
                        highest = VersionBump::Patch;
                    }
                    _ => {}
                }
            }
