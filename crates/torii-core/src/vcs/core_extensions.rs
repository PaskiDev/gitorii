...
impl GitRepo {
    ...
    pub fn rebase_with_todo(&self, _base: &str, _todo_file: &std::path::Path) -> Result<()> {
        Err(crate::error::ToriiError::RepoState("Interactive rebase with todo file requires a Unix shell. Not supported on this platform.".to_string()))
    }
...
// Porcelain tier (see `core::commit_inner`): raw `git2` params for the TUI.
#[doc(hidden)]
