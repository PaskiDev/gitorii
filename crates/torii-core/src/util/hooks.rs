...
use crate::toriignore::{glob_match, HookRules, SizeRules};
...
fn mark_trusted(repo: &Path, commands: &[String]) -> Result<()> {
    ...
    buf.push_str(&format!("\"{}\" = \"{}\"\n", key, hash));
    std::fs::write(&path, buf).map_err(|e| anyhow!("write {}: {}", path.display(), e))?;
    Ok(())
}
...
fn prompt_trust(repo: &Path, label: &str, commands: &[String]) -> Result<bool> {
    ...
    println!();
    println!(
        "⚠️  This repo defines {} hook(s) that will run via `sh -c`:",
        label
    );
    for cmd in commands {
        println!("     • {}", cmd);
...
pub fn check_size(rules: &SizeRules, repo: &Path, staged_paths: &[String]) -> Result<()> {
    if rules.max_bytes.is_none() && rules.warn_bytes.is_none() {
        return Ok(());
    }
    ...
    for rel in staged_paths {
        if rules.exclude.iter().any(|g| glob_match(rel, g)) {
            continue;
        }
        ...
        if let Some(max) = rules.max_bytes {
            if size > max {
                blocked.push((rel.clone(), size));
                continue;
            }
        }
        if let Some(warn) = rules.warn_bytes {
            if size > warn {
                warned.push((rel.clone(), size));
            }
        }
    }
